...
const NAV_SECTIONS = [
  ...
  { id: 'why', label: 'Why' },
  { id: 'case', label: 'Tim' },
  { id: 'problems', label: 'Leaks' },
  { id: 'flywheel', label: 'Engine' },
  { id: 'how', label: 'How' },
    ...
    <section id="team" className="relative bg-cream text-ink py-32">
      <div className="px-8 sm:px-16 max-w-7xl mx-auto">
        ...
        <div className="grid md:grid-cols-3 gap-6">
          {team.map((t, i) => (
            <Reveal key={t.name} delay={i * 0.1}>
              <div className="bg-paper border border-divider-lt rounded-2xl p-8 h-full">
                <p className="text-[10px] tracking-[0.25em] uppercase font-bold mb-6" style={{ color: t.color }}>
                  {t.tag}
                </p>
                <h3 className="font-display text-2xl tracking-tight mb-1">{t.name}</h3>
                <p className="text-mute text-sm mb-5">{t.role}</p>
                <p className="text-ink/80 text-sm leading-relaxed">{t.bio}</p>
              </div>
            </Reveal>
          ))}
        </div>
