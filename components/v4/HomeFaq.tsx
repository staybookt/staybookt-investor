...
const CSS = `
.hfq{padding:clamp(90px,12vw,150px) 0;background:var(--v4-cream,#f6f6f3);color:var(--v4-ink,#06080d);}
.hfq .wrap{width:100%;max-width:1080px;margin:0 auto;padding:0 clamp(20px,4vw,40px);}
...
.hfq h2{font-size:clamp(34px,4.6vw,60px);font-weight:600;letter-spacing:-.035em;line-height:1.0;margin:14px 0 0;max-width:9ch;}
.hfq h2 .g{background:var(--sb-grad);-webkit-background-clip:text;background-clip:text;color:transparent;}
.hfq .fa-p{margin-top:20px;font-size:16.5px;line-height:1.6;color:#6b7280;max-width:32ch;}
.hfq .fa-cta{display:inline-flex;align-items:center;gap:9px;margin-top:26px;background:var(--v4-ink,#06080d);color:#fff;font-size:15px;font-weight:600;border-radius:999px;padding:15px 28px;text-decoration:none;transition:gap .3s ease,transform .3s ease;}
.hfq .fa-cta:hover{gap:14px;transform:translateY(-1px);}

.hfq .list{display:flex;flex-direction:column;gap:8px;}
.hfq-q{--fc:#0ea5e9;position:relative;background:transparent;border:1px solid transparent;border-radius:18px;
  transition:background .35s ease,border-color .35s ease,box-shadow .35s ease,transform .35s ease;}
.hfq-q::after{content:'';position:absolute;left:clamp(16px,2vw,22px);right:clamp(16px,2vw,22px);bottom:0;height:1px;background:#e2e2dc;transition:opacity .3s ease;}
.hfq-q:last-child::after{opacity:0;}
.hfq-q:hover{background:rgba(255,255,255,.6);}
.hfq-q.open{background:#fff;border-color:#ececeb;box-shadow:0 26px 54px -34px rgba(6,12,20,.4);transform:translateY(-1px);}
...
.hfq-q .fk{display:block;font-size:11px;font-weight:700;letter-spacing:.16em;text-transform:uppercase;color:var(--fc);opacity:.85;}
.hfq-q .fq{display:block;margin-top:7px;font-size:clamp(17px,1.9vw,21px);font-weight:600;letter-spacing:-.02em;line-height:1.28;}
.hfq-q .pl{width:34px;height:34px;border-radius:50%;border:1px solid #dcdcd8;color:#8a8f98;
...
.hfq .fbody{max-height:0;overflow:hidden;transition:max-height .55s cubic-bezier(.16,1,.3,1);}
.hfq-q.open .fbody{max-height:520px;}
.hfq .fbody p{margin:0;padding:0 clamp(16px,2vw,22px) 0;font-size:16px;line-height:1.65;color:#52565e;max-width:62ch;}
...
@media(prefers-reduced-motion:reduce){.hfq .fbody,.hfq-q,.hfq-q .pl{transition:none;}}
`;
